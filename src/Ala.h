...
struct AlaMax
{
    ...
    inline AlaMax() __attribute__((always_inline))
    {
    }

    // allow construction from R, G, B
    inline AlaMax( float maxr, float maxg, float maxb)  __attribute__((always_inline))
...
*/
    ...
    uint32_t WS2811Color(AlaMax maxOut)
    {
        float newRed = float(r) * maxOut.r;
        float newGreen = float(g) * maxOut.g;
        float newBlue = float(b) * maxOut.b;

        return ((uint32_t(newRed) << 16) | (uint32_t(newGreen) << 8) | uint32_t(newBlue));
    }
